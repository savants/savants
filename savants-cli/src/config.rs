...
pub struct State {
    ...
    /// Cloud token (alternate key used by some flows)
    #[serde(default)]
...
impl State {
    ...
    pub fn cloud_token(&self) -> Option<String> {
        env::var("SAVANTS_TOKEN").ok()
            .or_else(|| env::var("SAVANTS_API_KEY").ok())
            .or_else(|| self.cloud_token.clone())
            .or_else(|| self.cloud_device_token.clone())
    }
