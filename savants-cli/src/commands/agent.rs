...
// ── Daemon management (savants agent start/stop/status) ──

/// Get the path for the daemon config based on OS
fn daemon_config_path() -> std::path::PathBuf {
    if cfg!(target_os = "macos") {
        dirs::home_dir().unwrap_or_default()
            .join("Library/LaunchAgents/com.savants.agent.plist")
    } else {
        // Linux systemd user service
        dirs::home_dir().unwrap_or_default()
            .join(".config/systemd/user/savants-agent.service")
    }
}

fn savants_binary_path() -> String {
    std::env::current_exe()
        .unwrap_or_else(|_| std::path::PathBuf::from("savants"))
        .to_string_lossy()
        .to_string()
}
...
pub async fn daemon_start() {
    ...
    println!("{}", "Setting up savants agent daemon...".bold());

    // 1. Check for cloud token
    let state = crate::config::State::load();
    let token = match state.cloud_token() {
        Some(t) => t,
        None => {
            eprintln!("{}: Not connected to cloud. Run: {}", "Error".red(), "savants connect cloud".cyan());
            std::process::exit(1);
        }
    };

    // 2. Auto-create agent key via cloud API
    let hostname = hostname::get()
        .map(|h| h.to_string_lossy().to_string())
        .unwrap_or_else(|_| "unknown".to_string());

    println!("  Host: {}", hostname.cyan());
    println!("  Registering agent key...");

    let client = reqwest::Client::new();
    let key_resp = client
        .post("https://api.savants.cloud/api/v1/agents/keys")
        .header("Authorization", format!("Bearer {}", token))
        .json(&serde_json::json!({"name": hostname}))
        .send()
        .await;

    let agent_key = match key_resp {
        Ok(resp) if resp.status().is_success() => {
            let body: serde_json::Value = resp.json().await.unwrap_or_default();
            body["key"].as_str().unwrap_or("").to_string()
        }
        Ok(resp) => {
            // Key might already exist, try to use existing token
            let _ = resp.text().await;
            println!("  Using existing cloud token as agent key");
            token.clone()
        }
        Err(e) => {
            eprintln!("{}: Cannot reach cloud: {}", "Error".red(), e);
            std::process::exit(1);
        }
    };

    // 3. Save agent key
    let key_path = dirs::home_dir().unwrap_or_default().join(".savants/agent_key");
    let _ = std::fs::write(&key_path, &agent_key);

    // 4. Install platform-specific daemon
    let binary = savants_binary_path();
    ...
    if cfg!(target_os = "macos") {
        ...
        // Generate app icon from SVG (creates ~/.savants/savants.icns)
        let home = dirs::home_dir().unwrap_or_default();
        let icon_path = home.join(".savants/savants.icns");
        if !icon_path.exists() {
            println!("  Generating app icon...");
            generate_macos_icon(&home);
        }
...
<dict>
    <key>Label</key>
    <string>com.savants.agent</string>
    ...
    <dict>
        <key>SAVANTS_TOKEN</key>
        <string>{token}</string>
        <key>SAVANTS_AGENT_KEY</key>
    ...
    <string>{home}/.savants/agent.log</string>
    <key>StandardErrorPath</key>
    <string>{home}/.savants/agent.err</string>
    ...
    <string>Background</string>
        ...
        std::fs::write(&config_path, &plist).unwrap_or_else(|e| {
            eprintln!("{}: Failed to write LaunchAgent: {}", "Error".red(), e);
            std::process::exit(1);
        });

        // Unload first (ignore errors if not loaded)
        let _ = std::process::Command::new("launchctl")
            .args(["unload", &config_path.to_string_lossy()])
            .output();

        // Load and start
        let load = std::process::Command::new("launchctl")
            .args(["load", &config_path.to_string_lossy()])
            .output();

        match load {
            Ok(o) if o.status.success() => {
                println!();
                println!("  {} Agent daemon installed and started", "✔".green());
                println!("  Runs in background, auto-restarts on reboot");
                println!("  Config: {}", config_path.display());
                println!("  Logs:   ~/.savants/agent.log");

                // Prompt for Full Disk Access on macOS (first time only)
                let fda_marker = home.join(".savants/.fda_prompted");
                if !fda_marker.exists() {
                    println!();
                    println!("  {} For full monitoring, grant Full Disk Access:", "!".yellow());
                    ...
                    let _ = std::process::Command::new("open")
                        ...
                        .output();
                    let _ = std::fs::write(&fda_marker, "prompted");
                }

                println!();
                println!("  View at: {}", "https://savants.cloud/dashboard".cyan());
                println!("  Stop:    {}", "savants agent stop".dimmed());
                println!("  Status:  {}", "savants agent status".dimmed());
            }
            Ok(o) => {
                eprintln!("{}: launchctl load failed: {}", "Error".red(),
...
Description=Savants Agent
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart={binary} agent run
Environment=SAVANTS_TOKEN={token}
Environment=SAVANTS_AGENT_KEY={agent_key}
Environment=PATH=/run/current-system/sw/bin:/usr/local/bin:/usr/bin:/bin:{home}/.local/bin
Restart=always
RestartSec=10
...
"#,
        ...
        std::fs::write(&config_path, &unit).unwrap_or_else(|e| {
            eprintln!("{}: Failed to write systemd unit: {}", "Error".red(), e);
            std::process::exit(1);
        });
        ...
        let _ = std::process::Command::new("systemctl")
            ...
            .output();
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "enable", "savants-agent"])
            .output();
        let start = std::process::Command::new("systemctl")
            .args(["--user", "start", "savants-agent"])
            .output();
        ...
        match start {
            Ok(o) if o.status.success() => {
                ...
                println!("  {} Agent daemon installed and started", "✔".green());
                println!("  Config: {}", config_path.display());
                println!("  Logs:   journalctl --user -u savants-agent -f");
                println!();
                println!("  View at: {}", "https://savants.cloud/dashboard".cyan());
                println!("  Stop:    {}", "savants agent stop".dimmed());
                println!("  Status:  {}", "savants agent status".dimmed());
            ...
            Ok(o) => {
                eprintln!("{}: systemctl start failed: {}", "Error".red(),
                    String::from_utf8_lossy(&o.stderr));
            }
            Err(e) => {
                eprintln!("{}: {}", "Error".red(), e);
...
pub fn daemon_stop() {
    ...
    if cfg!(target_os = "macos") {
        let result = std::process::Command::new("launchctl")
            ...
            .output();
        match result {
            Ok(o) if o.status.success() => println!("{} Agent stopped", "✔".green()),
            _ => println!("{} Agent not running or not installed", "·".dimmed()),
        }
    } else {
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "stop", "savants-agent"])
            .output();
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "disable", "savants-agent"])
            .output();
...
pub fn daemon_status() {
    ...
    if cfg!(target_os = "macos") {
        let result = std::process::Command::new("launchctl")
            .args(["list", "com.savants.agent"])
            .output();
        match result {
            Ok(o) if o.status.success() => {
                let output = String::from_utf8_lossy(&o.stdout);
                // Parse PID from launchctl output
                let running = !output.contains("-\t0\t");
                if running {
                    println!("{} Agent is {}", "●".green(), "running".green());
                } else {
                    println!("{} Agent is {}", "●".red(), "stopped".red());
                }
            }
            _ => {
                println!("{} Agent is {} (not installed)", "●".dimmed(), "not running".dimmed());
                println!("  Run {} to install", "savants agent start".cyan());
                return;
            }
        }
    } else {
        let result = std::process::Command::new("systemctl")
            .args(["--user", "is-active", "savants-agent"])
            .output();
        match result {
            Ok(o) => {
                ...
                if status == "active" {
                    ...
                    println!("{} Agent is {}", "●".red(), status.red());
                }
            }
            _ => {
                println!("{} Agent is {} (not installed)", "●".dimmed(), "not running".dimmed());
    ...
    let log_path = dirs::home_dir().unwrap_or_default().join(".savants/agent.log");
    if log_path.exists() {
        println!();
    ...
    println!();
    println!("  Config: {}", config_path.display());
    println!("  Dashboard: {}", "https://savants.cloud/dashboard".cyan());
}

/// Generate a macOS .icns icon from an embedded SVG.
#[allow(dead_code)]
fn generate_macos_icon(home: &std::path::Path) {
    let savants_dir = home.join(".savants");
    let _ = std::fs::create_dir_all(&savants_dir);

    // Write the SVG icon
    let svg_path = savants_dir.join("icon.svg");
    let svg = r##"<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
  <rect width="512" height="512" rx="96" fill="#0a0a0a"/>
  <text x="256" y="370" font-family="system-ui,-apple-system,Helvetica" font-weight="800" font-size="320" fill="#22d3ee" text-anchor="middle">S</text>
</svg>"##;
    let _ = std::fs::write(&svg_path, svg);

    // Convert SVG to PNG using sips (macOS built-in) or rsvg-convert
    let png_path = savants_dir.join("icon_512.png");
    let icns_path = savants_dir.join("savants.icns");

    // Try rsvg-convert first (better SVG support)
    let png_ok = std::process::Command::new("rsvg-convert")
        .args(["-w", "512", "-h", "512", "-o"])
        .arg(&png_path)
        .arg(&svg_path)
        .output()
    ...
    // Fallback: use Python to render SVG
    if !png_ok {
        let _ = std::process::Command::new("python3")
            .args(["-c", &format!(
                "from PIL import Image, ImageDraw, ImageFont; \
                 ...
                 try:\n  f = ImageFont.truetype('/System/Library/Fonts/SFNS.ttf', 320)\nexcept:\n  f = ImageFont.load_default()\n\
                 d.text((256, 256), 'S', fill=(34,211,238,255), anchor='mm', font=f); \
                 img.save('{}')", png_path.display()
            )])
            .output();
    }

    // If we have a PNG, try to convert to icns
    if png_path.exists() {
        // Create iconset directory
        let iconset = savants_dir.join("savants.iconset");
        let _ = std::fs::create_dir_all(&iconset);

        // Copy PNG at different sizes (sips can resize)
        for size in &[16, 32, 64, 128, 256, 512] {
            let dest = iconset.join(format!("icon_{}x{}.png", size, size));
        ...
        if icns_path.exists() {
            eprintln!("  Icon: {}", icns_path.display());
        }
    }
...
// ── Agent main loop (savants agent run) ──
...
pub async fn start(name: Option<String>) {
    ...
    // ── K8s watcher: real-time streaming to cloud ──
    if which("kubectl") {
        // Ensure SAVANTS_TOKEN is set for the cloud sink
        std::env::set_var("SAVANTS_TOKEN", &token);

        let cn = agent_name.clone();
        tokio::spawn(async move {
            tokio::time::sleep(std::time::Duration::from_secs(3)).await;

            // Create the cloud event sink directly (no FalkorDB needed)
            let cloud_url = std::env::var("SAVANTS_CLOUD_URL")
                .unwrap_or_else(|_| "https://api.savants.cloud".to_string());
            let api_key = std::env::var("SAVANTS_TOKEN").unwrap_or_default();
            let sink = std::sync::Arc::new(
                crate::k8s::cloud_sink::CloudEventSink::new(&cloud_url, &api_key, &cn)
            );

            loop {
                println!("[k8s-watch] Starting real-time watcher for '{}'", cn);
                let kube_result: Result<kube::Client, String> =
                    crate::k8s::ingestor::K8sIngestor::kube_client_from_kubeconfig(None).await
                        .map_err(|e| e.to_string());
                match kube_result {
                    Ok(kube_client) => {
                        // Snapshot via kubectl (no graph client needed)
                        use kube::api::{Api, ListParams};
                        use k8s_openapi::api::core::v1::Pod;
                        use k8s_openapi::api::apps::v1::Deployment;

                        let pods: usize = Api::<Pod>::all(kube_client.clone())
                            .list(&ListParams::default()).await
                            .map(|l| l.items.len()).unwrap_or(0);
                        let deploys: usize = Api::<Deployment>::all(kube_client.clone())
                            .list(&ListParams::default()).await
                            .map(|l| l.items.len()).unwrap_or(0);

                        println!("[k8s-watch] Snapshot: {} pods, {} deploys", pods, deploys);
                        sink.record("snapshot", "Cluster", &cn, "", "applied", "info",
                            &format!("pods={} deploys={}", pods, deploys));
                        sink.flush();

                        // Watch for changes and stream to cloud
                        use futures::TryStreamExt;
                        use kube::runtime::watcher::{self, Event};

                        let pod_api: Api<Pod> = Api::all(kube_client.clone());
                        let pod_sink = sink.clone();
                        let pod_cn = cn.clone();
                        let pod_handle = tokio::spawn(async move {
                            let stream = watcher::watcher(pod_api, watcher::Config::default());
                                ...
                                match event {
                                    Event::Applied(p) => {
                                        ...
                                        let phase = p.status.as_ref()
                                            .and_then(|s| s.phase.as_deref()).unwrap_or("Unknown");
                                        let restarts: i32 = p.status.as_ref()
                                            .and_then(|s| s.container_statuses.as_ref())
                                            .map(|cs| cs.iter().map(|c| c.restart_count).sum())
                                            .unwrap_or(0);
                                        let sev = if phase == "Failed" || restarts > 5 { "warning" } else { "info" };
                                        pod_sink.record("pod_applied", "Pod", name, ns, "applied", sev,
                                            &format!("phase={} restarts={}", phase, restarts));
                                    }
                                    Event::Deleted(p) => {
                                        let name = p.metadata.name.as_deref().unwrap_or("");
                                        let ns = p.metadata.namespace.as_deref().unwrap_or("");
                                        pod_sink.record("pod_deleted", "Pod", name, ns, "deleted", "warning", "");
                                    }
                                    Event::Restarted(_) => {}
                                }
                            }
                        ...
                        let deploy_api: Api<Deployment> = Api::all(kube_client.clone());
                        let deploy_sink = sink.clone();
                        let deploy_handle = tokio::spawn(async move {
                            let stream = watcher::watcher(deploy_api, watcher::Config::default());
                            futures::pin_mut!(stream);
                            while let Ok(Some(event)) = stream.try_next().await {
                                match event {
                                    Event::Applied(d) => {
                                        ...
                                        let sev = if ready < desired { "warning" } else { "info" };
                                        deploy_sink.record("deploy_applied", "Deployment", name, ns, "applied", sev,
                                            &format!("ready={}/{}", ready, desired));
                                    }
                                    Event::Deleted(d) => {
                                        let name = d.metadata.name.as_deref().unwrap_or("");
                                        let ns = d.metadata.namespace.as_deref().unwrap_or("");
                                        deploy_sink.record("deploy_deleted", "Deployment", name, ns, "deleted", "warning", "");
                                    }
                                    Event::Restarted(_) => {}
                                }
                            }
                        ...
                        // Wait for either watcher to die
                        tokio::select! {
                            _ = pod_handle => println!("[k8s-watch] Pod watcher stopped"),
                            _ = deploy_handle => println!("[k8s-watch] Deploy watcher stopped"),
                        }

                        sink.flush();
                    }
                    Err(e) => {
                        println!("[k8s-watch] k8s connect failed: {}", e);
                    }
                }
                println!("[k8s-watch] Restarting in 10s...");
                tokio::time::sleep(std::time::Duration::from_secs(10)).await;
            }
        });
    }

    // ── ISP monitor: continuous network quality tracking ──
    std::thread::spawn(|| {
        super::isp_monitor::run_monitor();
    });
    ...
    loop {
        ...
        if last_watch.elapsed().as_secs() >= WATCH_INTERVAL_SECS {
            ...
            // Persist ALL findings (not just new) as graph_events for host_story timeline
            if !findings.is_empty() {
                let events: Vec<serde_json::Value> = findings.iter()
                    .filter(|f| f.severity == "critical" || f.severity == "warning")
                    .map(|f| serde_json::json!({
                        "type": format!("host_{}", f.category),
                        "resource_type": "Host",
                        "name": agent_name,
                        "namespace": "",
                        "action": f.key,
                ...
                if !events.is_empty() {
                    let _ = client
                        .post(format!("{}/api/v1/events/k8s", cloud_url))
                        .header("Authorization", format!("Bearer {}", token))
                        .json(&serde_json::json!({
                            "cluster": agent_name,
                            "events": events,
...
fn watch_health() -> Vec<Finding> {
    ...
    // ── Kernel-level health (dmesg errors, filesystem state, SMART, I/O) ──
    findings.extend(watch_kernel_health());
    ...
    // ── K8s service health (silent failures like bichon sync stall) ──
    findings.extend(watch_service_health());

    // ── Cloud providers ──
    findings.extend(watch_cloud_providers());

    // ── Memory (cross-platform) ──
    let (mem_total_kb, mem_avail_kb) = if cfg!(target_os = "macos") {
        // macOS: use sysctl and vm_stat
        let total: u64 = std::process::Command::new("sysctl").args(["-n", "hw.memsize"]).output().ok()
            .and_then(|o| String::from_utf8_lossy(&o.stdout).trim().parse::<u64>().ok())
            .unwrap_or(0) / 1024; // bytes -> KB
        let free: u64 = std::process::Command::new("vm_stat").output().ok()
            .map(|o| {
                let out = String::from_utf8_lossy(&o.stdout);
                let page_size: u64 = 16384; // ARM64 macOS page size
                let free_pages: u64 = out.lines()
                    .filter(|l| l.contains("Pages free") || l.contains("Pages inactive") || l.contains("Pages speculative"))
                    .filter_map(|l| l.rsplit(':').next()?.trim().trim_end_matches('.').parse::<u64>().ok())
                    .sum();
    ...
    if mem_total_kb > 0 {
        let pct = (mem_total_kb - mem_avail_kb) as f64 / mem_total_kb as f64 * 100.0;
        if pct >= MEMORY_CRIT_PCT {
            findings.push(Finding {
                ...
                message: format!("Memory at {:.0}% ({} / {} MB). Risk of OOM.", pct, (mem_total_kb - mem_avail_kb) / 1024, mem_total_kb / 1024),
                metadata: serde_json::json!({"percent": pct, "used_mb": (mem_total_kb - mem_avail_kb) / 1024, "total_mb": mem_total_kb / 1024}),
            });
            ...
            findings.push(Finding {
                ...
                message: format!("Memory at {:.0}% ({} / {} MB).", pct, (mem_total_kb - mem_avail_kb) / 1024, mem_total_kb / 1024),
    ...
    // ── Load (cross-platform) ──
    {
        let (load_1m, cores) = if cfg!(target_os = "macos") {
            let load: f64 = std::process::Command::new("sysctl").args(["-n", "vm.loadavg"]).output().ok()
                .and_then(|o| {
                    let s = String::from_utf8_lossy(&o.stdout);
                    s.trim().trim_start_matches("{ ").split_whitespace().next()?.parse().ok()
                })
                .unwrap_or(0.0);
            let cores: f64 = std::process::Command::new("sysctl").args(["-n", "hw.ncpu"]).output().ok()
                .and_then(|o| String::from_utf8_lossy(&o.stdout).trim().parse().ok())
                .unwrap_or(1.0);
            (load, cores)
        } else {
            let load: f64 = std::fs::read_to_string("/proc/loadavg").ok()
                .and_then(|s| s.split_whitespace().next()?.parse().ok())
                .unwrap_or(0.0);
            let cores: f64 = std::fs::read_to_string("/proc/cpuinfo").ok()
    ...
    // ── Disk (cross-platform) ──
    {
        let df_args: &[&str] = if cfg!(target_os = "macos") {
            &["-h"]  // macOS df
        } else {
            &["--output=target,pcent", "-x", "tmpfs", "-x", "devtmpfs"]  // Linux df
        };

        if let Ok(out) = std::process::Command::new("df").args(df_args).output() {
            if out.status.success() {
                let raw = String::from_utf8_lossy(&out.stdout);
                for line in raw.lines().skip(1) {
                    let parts: Vec<&str> = line.split_whitespace().collect();
                    // Linux format: mount pct | macOS format: fs size used avail pct mount
                    let (mount, pct_str) = if cfg!(target_os = "macos") {
                        if parts.len() >= 9 {
                            (parts[8], parts[4]) // mount is last, pct is 5th
                        } else { continue; }
                    } else {
                    ...
                    if mount.starts_with("/dev") && mount != "/" { continue; }
                    if mount.contains("devfs") || mount.contains("tmpfs") { continue; }
                    // macOS: skip system snapshot volumes, recovery, preboot, VM, xarts
                    if mount.starts_with("/System/Volumes/") && mount != "/System/Volumes/Data" { continue; }
                    if mount == "/System/Volumes/Data/home" { continue; } // automount
                    // Only alert on / and /System/Volumes/Data (the real user disk)
                    ...
                    if pct >= DISK_CRIT_PCT {
                        findings.push(Finding {
                            ...
                            message: format!("Disk {} is {}% full. Immediate action required.", mount, pct),
...
/// Generic service health monitor - auto-discovers all running services,
/// checks their logs for error spikes, and detects silent failures.
/// No hardcoded service names - works for any k8s cluster.
fn watch_service_health() -> Vec<Finding> {
    let mut findings = Vec::new();

    if !which("kubectl") { return findings; }

    // Auto-discover all deployments across all namespaces
    let deploy_out = std::process::Command::new("kubectl")
        .args(["get", "deploy", "-A", "--no-headers",
               "-o", "custom-columns=NS:.metadata.namespace,NAME:.metadata.name,READY:.status.readyReplicas,DESIRED:.spec.replicas"])
        .output();

    let deploys = match deploy_out {
        Ok(o) if o.status.success() => String::from_utf8_lossy(&o.stdout).to_string(),
        _ => return findings,
    };

    // Skip system namespaces that are noisy but not user services
    let skip_ns = ["kube-system", "kube-public", "kube-node-lease", "cert-manager",
                   "crossplane-system", "openebs", "velero", "monitoring", "trivy-system"];

    for line in deploys.lines() {
        let parts: Vec<&str> = line.split_whitespace().collect();
        if parts.len() < 4 { continue; }
        let ns = parts[0];
        let name = parts[1];
        let ready: i32 = parts[2].parse().unwrap_or(-1);
        let desired: i32 = parts[3].parse().unwrap_or(0);

        if skip_ns.contains(&ns) { continue; }
        if desired == 0 { continue; } // scaled to zero intentionally

        // Check pod logs for error lines (generic - no hardcoded patterns)
        if let Ok(log_out) = std::process::Command::new("kubectl")
            .args(["logs", "-n", ns, &format!("deploy/{}", name), "--since=10m", "--tail=100"])
            .output()
        {
            if log_out.status.success() {
                let logs = String::from_utf8_lossy(&log_out.stdout);
                // Count lines containing common error indicators
                let error_count = logs.lines()
                    .filter(|l| {
                        let lower = l.to_lowercase();
                        (lower.contains("error") || lower.contains("fatal") || lower.contains("panic")
                         || lower.contains("failed") || lower.contains("connection refused")
                         || lower.contains("name resolution") || lower.contains("timeout"))
                        && !lower.contains("no error") && !lower.contains("error=0")
                        && !lower.contains("error_count=0")
                    })
                    .count();

                // Alert if error rate is high (>10 errors in 10 min = something wrong)
                if error_count > 10 {
                    findings.push(Finding {
                        key: format!("svc_errors_{}_{}", ns, name),
                        severity: if error_count > 30 { "critical" } else { "warning" }.into(),
                        category: "service".into(),
                        title: format!("{}/{}: {} errors in last 10min", ns, name, error_count),
                        message: format!(
                            "Service {}/{} is running but logging {} error lines in the last 10 minutes. \
                             This may indicate a connectivity issue, dependency failure, or misconfiguration.",
                            ns, name, error_count
                        ),
                        metadata: serde_json::json!({
                            "namespace": ns, "service": name,
                            "error_count": error_count, "ready": ready, "desired": desired,
                        }),
                    });
                }
            }
    ...
    // DNS dependency check - if DNS fails, flag that ALL external-dependent services are impacted
    let dns_ok = std::process::Command::new("sh")
        .args(["-c", "nslookup google.com >/dev/null 2>&1"])
        .output()
        .map(|o| o.status.success())
        .unwrap_or(false);
    ...
    if !dns_ok {
        findings.push(Finding {
            key: "dns_impacts_services".into(),
            severity: "critical".into(),
            category: "service".into(),
            title: "DNS failure - all services depending on external APIs are impacted".into(),
            message: "DNS resolution is failing. Any service that connects to external endpoints \
                     (email servers, OAuth providers, cloud APIs, package registries) will fail silently. \
                     Fix the underlying network/DNS issue to restore service functionality.".into(),
            metadata: serde_json::json!({"dns_ok": false}),
        });
    }
...
/// Capture system state during a critical event (I/O pressure, read-only fs).
/// This gets sent to the cloud so we know what was happening BEFORE a crash.
fn capture_pre_crash_state() -> String {
        ...
        .output()
    {
        if out.status.success() {
            let raw = String::from_utf8_lossy(&out.stdout);
            dump.push("TOP PROCESSES BY CPU:".to_string());
            for line in raw.lines().take(10) {
                dump.push(format!("  {}", line.chars().take(120).collect::<String>()));
            }
        }
    }
        ...
        .output()
    {
        if out.status.success() {
            let raw = String::from_utf8_lossy(&out.stdout);
            dump.push("\nTOP PROCESSES BY MEMORY:".to_string());
            for line in raw.lines().take(10) {
                dump.push(format!("  {}", line.chars().take(120).collect::<String>()));
            }
        }
    ...
    // I/O stats per process (if iotop or /proc/*/io available)
    if let Ok(out) = std::process::Command::new("sh")
        .args(["-c", "for pid in $(ls /proc/ | grep '^[0-9]' | head -30); do \
            name=$(cat /proc/$pid/comm 2>/dev/null || echo '?'); \
            io=$(cat /proc/$pid/io 2>/dev/null | grep -E 'write_bytes|read_bytes' | head -2); \
            if [ -n \"$io\" ]; then echo \"$pid $name: $io\"; fi; done | head -15"])
        .output()
    {
        if out.status.success() {
            let raw = String::from_utf8_lossy(&out.stdout);
            if !raw.trim().is_empty() {
                dump.push("\nI/O PER PROCESS:".to_string());
    ...
    // Disk queue depth
    if let Ok(diskstats) = std::fs::read_to_string("/proc/diskstats") {
        dump.push("\nDISK QUEUE:".to_string());
        for line in diskstats.lines() {
            let parts: Vec<&str> = line.split_whitespace().collect();
            if parts.len() >= 14 {
                let name = parts[2];
                let ios_in_progress = parts[11];
                if name.starts_with("nvme") || name.starts_with("sd") {
                    if !name.contains('p') || name.ends_with("n1") {
                        dump.push(format!("  {} in_flight={}", name, ios_in_progress));
                    }
                }
            }
        }
    ...
    if let Ok(out) = std::process::Command::new("sh")
        .args(["-c", "cat /proc/sys/fs/file-nr"])
        .output()
    {
        if out.status.success() {
            let raw = String::from_utf8_lossy(&out.stdout);
            dump.push(format!("\nFILE DESCRIPTORS: {}", raw.trim()));
        }
    }

    // Recent dmesg errors (last 10)
        ...
        .output()
    {
        if out.status.success() {
            ...
            if !lines.is_empty() {
                dump.push("\nRECENT KERNEL ERRORS:".to_string());
    ...
    if let Ok(pressure) = std::fs::read_to_string("/proc/pressure/memory") {
        dump.push(format!("\nMEMORY PRESSURE: {}", pressure.lines().next().unwrap_or("?")));
    }

    dump.join("\n")
}

/// Kernel-level health checks that would catch read-only filesystems,
/// disk failures, USB errors, and I/O degradation.
fn watch_kernel_health() -> Vec<Finding> {
    let mut findings = Vec::new();

    // Skip on macOS - these are Linux-specific
    if cfg!(target_os = "macos") {
        return findings;
    }

    // ── 1. Filesystem mount state: detect read-only remounts ──
    if let Ok(mounts) = std::fs::read_to_string("/proc/mounts") {
        for line in mounts.lines() {
            ...
            // Only check real filesystems (skip proc, sys, cgroup, etc)
            if !parts[0].starts_with("/dev") { continue; }

            // Skip intentionally read-only mounts (NixOS store, snap squashfs)
            if mount_point == "/nix/store" || mount_point.starts_with("/snap/") || parts[2] == "squashfs" {
                continue;
            }

            if options.contains("ro,") || options.starts_with("ro,") || options == "ro" {
                let state_dump = capture_pre_crash_state();
                findings.push(Finding {
                    key: format!("fs_readonly_{}", mount_point),
                    severity: "critical".into(),
                    category: "filesystem".into(),
                    title: format!("CRITICAL: {} is mounted READ-ONLY", mount_point),
                    message: format!(
                        "Filesystem {} (device {}) is mounted read-only. This usually means the kernel \
                         detected I/O errors or filesystem corruption and remounted for safety. \
                         Data cannot be written. Immediate investigation required.\n\n\
                         STATE DUMP:\n{}",
                        mount_point, parts[0], state_dump
                    ),
                    metadata: serde_json::json!({
    ...
    // ── 2. dmesg kernel errors (recent, last 120 seconds) ──
    if let Ok(out) = std::process::Command::new("dmesg")
        .args(["--time-format", "reltime", "--level", "err,crit,alert,emerg"])
        .output()
    {
        if out.status.success() {
            let raw = String::from_utf8_lossy(&out.stdout);
            ...
            for line in &lines {
                let lower = line.to_lowercase();
                if lower.contains("usb") && (lower.contains("error") || lower.contains("disconnect") || lower.contains("device descriptor read")) {
                    usb_errors += 1;
                }
                if lower.contains("i/o error") || lower.contains("blk_update_request") || lower.contains("buffer i/o error") {
                    io_errors += 1;
                }
                if lower.contains("ext4") || lower.contains("btrfs") || lower.contains("xfs") || lower.contains("remounting filesystem read-only") {
                    fs_errors += 1;
                }
                if lower.contains("out of memory") || lower.contains("oom") || lower.contains("killed process") {
                    oom_kills += 1;
                }
                if samples.len() < 5 {
                    samples.push(line.chars().take(120).collect());
                }
            }

            if usb_errors > 0 {
                findings.push(Finding {
                    key: "dmesg_usb_errors".into(),
                    severity: if usb_errors > 10 { "critical" } else { "warning" }.into(),
                    category: "hardware".into(),
                    title: format!("{} USB errors in kernel log", usb_errors),
                    message: format!(
                        "{} USB-related errors detected in dmesg. Failing USB devices can cause \
                         system instability, filesystem corruption, and read-only remounts. \
                         Check `dmesg | grep -i usb` for details.",
                        usb_errors
                    ),
                    metadata: serde_json::json!({"count": usb_errors, "samples": samples}),
                });
            }

            if io_errors > 0 {
                findings.push(Finding {
                    key: "dmesg_io_errors".into(),
                    severity: "critical".into(),
                    category: "disk".into(),
                    title: format!("{} I/O errors in kernel log", io_errors),
                    message: format!(
                        "{} disk I/O errors detected. This indicates disk hardware failure, \
                         bad sectors, or controller issues. The kernel may remount filesystems \
                         read-only to protect data. Check SMART status immediately.",
                        io_errors
                    ),
                    metadata: serde_json::json!({"count": io_errors}),
                });
            }

            if fs_errors > 0 {
                findings.push(Finding {
                    key: "dmesg_fs_errors".into(),
                    severity: "critical".into(),
                    category: "filesystem".into(),
                    title: format!("{} filesystem errors in kernel log", fs_errors),
                    message: format!(
                        "{} filesystem errors detected in dmesg. This may indicate corruption. \
                         Run fsck on the affected filesystem after unmounting.",
                        fs_errors
                    ),
                    metadata: serde_json::json!({"count": fs_errors}),
                });
            }

            if oom_kills > 0 {
                findings.push(Finding {
                    key: "dmesg_oom_kills".into(),
                    severity: "critical".into(),
                    category: "memory".into(),
                    title: format!("{} OOM kills in kernel log", oom_kills),
                    message: format!(
                        "The kernel killed {} process(es) due to out-of-memory condition. \
                         This indicates severe memory pressure. Check which processes are \
                         consuming the most memory.",
                        oom_kills
                    ),
                    metadata: serde_json::json!({"count": oom_kills}),
                });
            }
        }
    }
    ...
    // ── 3. SMART disk health ──
    // Try nvme first (modern SSDs), fall back to smartctl
    for disk in &["/dev/nvme0n1", "/dev/sda"] {
        if !std::path::Path::new(disk).exists() { continue; }

        let tool = if disk.contains("nvme") { "nvme" } else { "smartctl" };
        let args: Vec<&str> = if disk.contains("nvme") {
            vec!["smart-log", disk]
        } else {
            vec!["-A", "-H", disk]
        };
            ...
            .output()
        {
            if out.status.success() {
                ...
                // NVMe: check critical warning, available spare, temperature
                if disk.contains("nvme") {
                    for line in raw.lines() {
                        let lower = line.to_lowercase();
                        if lower.contains("critical_warning") || lower.contains("critical warning") {
                            let val = line.split(':').nth(1).map(|s| s.trim()).unwrap_or("0");
                            if val != "0" && val != "0x0" {
                                findings.push(Finding {
                                    key: format!("smart_critical_{}", disk),
                                    severity: "critical".into(),
                                    category: "disk".into(),
                                    title: format!("NVMe SMART critical warning on {}", disk),
                                    message: format!("NVMe critical warning flag is non-zero ({}). Disk may be failing.", val),
                                    metadata: serde_json::json!({"disk": disk, "warning": val}),
                                });
                            }
                        }
                        if lower.contains("available spare") && !lower.contains("threshold") {
                            if let Some(pct_str) = line.split(':').nth(1) {
                                let pct: u32 = pct_str.trim().trim_end_matches('%').parse().unwrap_or(100);
                                if pct < 10 {
                                    findings.push(Finding {
                                        key: format!("smart_spare_{}", disk),
                                        severity: "critical".into(),
                                        category: "disk".into(),
                                        title: format!("NVMe spare capacity low: {}% on {}", pct, disk),
                                        message: format!("Available spare blocks at {}%. Disk is nearing end of life.", pct),
                                        metadata: serde_json::json!({"disk": disk, "spare_pct": pct}),
                                    });
                                }
                            }
                        }
                        if lower.contains("temperature") && !lower.contains("threshold") && !lower.contains("warning") {
                    ...
                    // SATA/smartctl: check overall health
                    if raw.contains("FAILED") || raw.contains("FAILING_NOW") {
                        findings.push(Finding {
                            key: format!("smart_failed_{}", disk),
                            severity: "critical".into(),
                            category: "disk".into(),
    ...
    // ── 4. I/O wait pressure (precedes read-only remounts) ──
    if let Ok(pressure) = std::fs::read_to_string("/proc/pressure/io") {
        ...
        for line in pressure.lines() {
            if line.starts_with("some") {
                if let Some(avg10_str) = line.split("avg10=").nth(1) {
                    ...
                    if avg10 > 50.0 {
                        // Pre-crash state dump: capture what's consuming I/O RIGHT NOW
                        let state_dump = capture_pre_crash_state();

                        findings.push(Finding {
                            key: "io_pressure_critical".into(),
                            severity: "critical".into(),
                            category: "disk".into(),
                            title: format!("I/O pressure critical: {:.0}%", avg10),
                            message: format!(
                                "I/O pressure at {:.0}% (10-second average). Tasks are stalling waiting for disk. \
                                 This often precedes filesystem read-only remounts.\n\n\
                                 PRE-CRASH STATE DUMP:\n{}",
                                avg10, state_dump
                            ),
                            metadata: serde_json::json!({"avg10": avg10, "state_dump": state_dump}),
                        });
                    } else if avg10 > 20.0 {
                        findings.push(Finding {
                            key: "io_pressure_high".into(),
                            severity: "warning".into(),
                            category: "disk".into(),
                            title: format!("I/O pressure high: {:.0}%", avg10),
                            message: format!("I/O pressure at {:.0}%. Disk may be overloaded.", avg10),
                            metadata: serde_json::json!({"avg10": avg10}),
                        });
                    }
                }
            }
    ...
    // ── 5. Unclean shutdown detection ──
    // Check if the last boot was a clean shutdown or crash
    if let Ok(out) = std::process::Command::new("last")
        .args(["-x", "shutdown", "reboot", "-1"])
        .output()
    {
        if out.status.success() {
            let raw = String::from_utf8_lossy(&out.stdout);
            // If there's a "reboot" without a preceding "shutdown", it was a crash
            let has_reboot = raw.lines().any(|l| l.contains("reboot"));
            let has_shutdown = raw.lines().any(|l| l.contains("shutdown"));
            if has_reboot && !has_shutdown {
                findings.push(Finding {
                    key: "unclean_shutdown".into(),
                    severity: "warning".into(),
                    category: "system".into(),
                    title: "Last boot was from unclean shutdown (crash/power loss)".into(),
                    message: "The system was rebooted without a clean shutdown. This may indicate a kernel panic, \
                             power failure, or forced reboot due to unresponsive system.".into(),
                    metadata: serde_json::json!({"detected_from": "last -x"}),
                });
            }
        }
    }
